...
class Learner:
    ...
    def learn(self, game):
        ...
        for i in range(self.num_iter):
            ...
            R = self.Q[pos_a, pos_b, ball_owner].T
            a = cp.Variable(self.n_action)
            vs = [cp.sum(a @ R[k]) for k in range(self.n_action)]
            obj = cp.Maximize(cp.minimum(*vs))
            constraints = [cp.sum(a) == 1, a >= 0]
            prob = cp.Problem(obj, constraints)
            V = prob.solve(solver=cp.GLPK)
            ...
            self.Qb[pos_a, pos_b, ball_owner, action_a, action_b] = \
                ...
                self.alpha * ((1 - self.gamma) * reward_b + self.gamma * -V)
            ...
            if (pos_a, pos_b, ball_owner, action_a, action_b) == (1, 5, 0, 1, 4):
                ...
                self.a_c.append(np.argmax(self.Q[1,5,0]))
